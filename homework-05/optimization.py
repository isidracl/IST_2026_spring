...
class LineSearchTool(object):
    ...
    def line_search(self, oracle, x_k, d_k, previous_alpha=None):
        ...
        if self._method == 'Constant':
            ...
            result = scalar_search_wolfe2(phi, dphi, c1=self.c1, c2=self.c2)
            alpha = result[0]
            if alpha is None:
                alpha = self.alpha_0
                while phi(alpha) > phi0 + self.c1 * alpha * dphi0:
...
def gradient_descent(oracle, x_0, tolerance=1e-5, max_iter=10000,
    ...
    for k in range(max_iter + 1):
        ...
        if grad_0_norm_sq is None:
            grad_0_norm_sq = grad_norm_sq
        ...
        if display:
            print(f'iter={k}, f={f_k:.6f}, ||grad||={np.sqrt(grad_norm_sq):.6f}')
        ...
        d_k = -g_k
        alpha_prev = line_search_tool.line_search(oracle, x_k, d_k, alpha_prev)
        x_k = x_k + alpha_prev * d_k
    return x_k, 'success', history
...
def newton(oracle, x_0, tolerance=1e-5, max_iter=100,
    ...
    for k in range(max_iter + 1):
        ...
        H_k = oracle.hess(x_k)
        try:
            cho = scipy.linalg.cho_factor(H_k)
